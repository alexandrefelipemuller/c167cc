...
- **32-bit integers** (`int32_t`/`uint32_t`) can be declared and
  ...
  `IR_DIV32_MUL`): `dst32 = a * b` (direct assignment of a widening
  multiply to a 32-bit variable), `some32bitvar >> N` for constant `N`,
  `some32bitvar / expr` / `some32bitvar % expr` (16-bit divisor, result
  truncated to 16 bits), and `((T)a * (T)b) / expr` / `% expr` (an inline
  widening product used directly as dividend, not first assigned to a
  32-bit temp). Everything else - `+`, `-`, `<<` used to compose a 32-bit
  value, compound assignment (`+=` etc.), a product added to a 32-bit
  accumulator in the same expression, 32-bit values threaded through
  anything but those four exact shapes, function arguments/returns - is
  still silently wrong. Avoid general 32-bit arithmetic until this is
  addressed.
